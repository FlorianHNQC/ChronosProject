...
import type { Competition, Match, Player, Team } from "@shared/schema";
...
/**
 * Console admin — engagements de drifter par match (tarification modulaire).
 */
export function DriftersAdminPage() {
  ...
  return (
    <div className="w-full px-6 py-8">
      <h1 className="text-2xl font-bold mb-6">Drifters</h1>

      <div className="flex flex-wrap items-center gap-2 mb-6">
        <select value={competitionId} onChange={(e) => { setCompetitionId(e.target.value); setMatchId(""); }} className="h-9 rounded-md border bg-background px-2 text-sm">
          <option value="">— compétition —</option>
          {(comps ?? []).map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}
        </select>
        {competitionId && (
          <select value={matchId} onChange={(e) => setMatchId(e.target.value)} className="h-9 rounded-md border bg-background px-2 text-sm max-w-xs">
...
export default DriftersAdminPage;
