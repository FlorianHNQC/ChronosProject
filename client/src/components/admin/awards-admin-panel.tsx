...
/** Éditeur de récompenses scopé à une compétition (utilisé dans la page de gestion). */
export function AwardsAdminPanel({ competitionId }: { competitionId: string }) {
  const { toast } = useToast();
  const { data: players } = useQuery<Player[]>({ queryKey: ["/api/players"] });
  const { data: allCards } = useQuery<AwardCard[]>({ queryKey: ["/api/palmares"] });
  const cards = (allCards ?? []).filter((c) => c.competitionId === competitionId);
  ...
  return (
    <div className="max-w-4xl">
      <Card className="p-5 mb-8 space-y-3">
        ...
        {recipientType === "team" && (
          <select value={teamId} onChange={(e) => setTeamId(e.target.value)} className="w-full h-9 rounded-md border bg-background px-2 text-sm">
      ...
      <div className="space-y-2">
        ...
        {cards.length === 0 && <p className="text-sm text-muted-foreground">Aucune récompense pour cette compétition.</p>}
      </div>
    </div>
  );
}

export default AwardsAdminPanel;
